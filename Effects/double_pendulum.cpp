...
namespace dp {
    ...
    double g  = 9.81;
    double dt = 5e-3; //Resolution of Runge-Kutta;
    ...
    dp::xy first_edge(const state& st, const system& ss) {
        xy first_edge {
    ...
    // Calculations on position of edges
    xy second_edge(const state& st, const system& ss) {
        xy fe = first_edge(st,ss);

        xy second_edge{
            fe.first  + ss.length.second * sin(st.theta.second),
            fe.second + ss.length.second * cos(st.theta.second)
          };
